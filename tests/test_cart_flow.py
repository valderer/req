...
class TestCartFlow:
    ...
    @allure.title("数量为 0 的商品进入失效购物车")
    def test_zero_quantity_enters_invalid_cart(
        self,
        login_authorization,
        cart_api,
        cart_data,
        ...
        assert add_response.status_code in (200, 201)
        cart_id = add_response.json().get("data", {}).get("cart_id")
        assert cart_id, "数量为 0 时没有返回 cart_id"
        cart_cleanup.append(
        ...
        assert list_response.status_code == 200
        invalid_cart = find_cart(
            list_response.json()["data"],
            cart_id,
            section="invalid_cart",
        )
        assert invalid_cart is not None, (
            f"数量为 0 的商品没有进入 invalid_cart，cart_id={cart_id}"
        )
        assert invalid_cart["num"] == 0
